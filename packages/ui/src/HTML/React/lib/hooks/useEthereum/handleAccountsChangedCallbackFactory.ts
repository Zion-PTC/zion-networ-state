...
export const handleAccountsChangedCallbackFactory: HandleAccountsChangedCallbackFactory =
  function (setIsConnected, setButtonMess, provider) {
    return function (accounts) {
      if (!accounts.length) setIsConnected(false);
      if (accounts.length)
        getSignerAddress(provider.getSigner(), [setButtonMess], setIsConnected);
    };
  };
