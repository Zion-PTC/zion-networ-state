...
export type MetamaskUseState = MetaMaskEthereumProvider | undefined;
export type ContractUseState = ethers.Contract | undefined;
export type ProviderUseState = ethers.providers.Web3Provider | undefined;

export type WindowEthRequired = Required<Pick<typeof window, "ethereum">> &
...
export interface UseEthereum {
  ...
  setHandleClick: Dispatch<
    SetStateAction<(e: MouseEvent<HTMLButtonElement>) => void>
  >;
}

export type useEthereumData = {
...
export const useEthereum = (
  data: useEthereumData = {
    connectMetamaskMessage: "Connect Metamask",
    metamaskNotInstalled: "no Metamask",
    contractAddress: "0x7c544aC11d219d10Bcd3ef2A048f4a72588a2d59",
  },
  noMetamaskClick: (e: MouseEvent<HTMLButtonElement>) => void = function () {
    console.log("clicked");
  }
): UseEthereum => {
  const [metamask, setMetamask] = useState<MetamaskUseState>(undefined);
  const [contract, setContract] = useState<ContractUseState>(undefined);
  const [provider, setProvider] = useState<ProviderUseState>(undefined);
  const [evm, setEvm] = useState<IevmUseState>(undefined);
  const [isMetamask, setIsMetamask] = useState<boolean>(false);
  const [buttonMess, setButtonMess] = useState("loading...");
  const [signerAddress, setSignerAddress] = useState<string | undefined>(
    undefined
  );
  const [isConnected, setIsConnected] = useState(false);
  const [handleClick, setHandleClick] = useState(() => noMetamaskClick);

  const listAccountsCallbacks = listAccountsCallbacksFactory(setIsConnected);

  const handleAccountsChangedCallbacks = handleAccountsChangedCallbackFactory(
    setIsConnected,
    setButtonMess,
    provider
  );

  const handleAccountsChanged = handleAccountsChangedFactory(
    listAccounts,
    provider,
  ...
  useEffect(() => {
    detect(window, detectEthereumProvider)
      ...
      .catch(() => setButtonMess(data.metamaskNotInstalled));
  });

  useEffect(() => {
    if (!isMetamask) return;
    const evm = new EVMweb({ window: window as WindowEthRequired });
    const factory = evm.contractFactories.ERC1155IndividualURI;
  ...
  useEffect(() => {
    if (!isConnected && metamask && data.connectMetamaskMessage)
      setButtonMess(data.connectMetamaskMessage);
    if (isConnected && evm)
      getSignerAddress(evm.signer, [setButtonMess, setSignerAddress]);
