...
function calculateSizeOnHeight(
  ...
  if (options.base === "width")
    throw new Error("width was given as base, expected 'height'");
...
function calculateSizeOnWidth(
  ...
  if (options.base === "height")
    throw new Error("height was given as base, expected 'width'");
...
export function calculateSize(ratio: number, height: number): FSI;
export function calculateSize(
  ratio: number,
  base: number,
  options?: CalculateSizeOptions
  ...
  switch (options) {
    ...
    case undefined:
      result = calculateSizeOnHeight(ratio, base, { base: "height" });
      break;
